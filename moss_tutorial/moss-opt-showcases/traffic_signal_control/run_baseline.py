...
def main():
    ...
    eng = Engine(
	    name="repo",
        map_file=f'{args.data}/map.pb',
        person_file=f'{args.data}/sumo_persons.routed.pb',
    )
    print('done.')
    print([x for x in dir(eng) if "count" in x.lower() or "finished" in x.lower() or "running" in x.lower()])
    ...
    with tqdm(range(args.steps), ncols=100) as bar:
        for step in bar:
            ...
            recorder.record()
            bar.set_description(
                f"Running: {eng.get_running_person_count()} "
                f"Finished: {eng.get_finished_person_count()} "
                f"Avg_time: {eng.get_finished_person_average_traveling_time():.1f}s"
            )
